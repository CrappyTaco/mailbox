...
for (const deployedOrigin of [
  'https://auggieisromantic.uk',
  'https://mailbox.aselke2002.workers.dev',
])
  void test(
    `${deployedOrigin}: browser send moves both worlds and persists after reload`,
    { timeout: 120000 },
    async () => {
      ...
      try {
        ...
        for (const [owner, sender, recipient] of [
          ...
          assert.equal(
            await db
              .prepare(
                'SELECT latest_id FROM mailbox_world WHERE singleton = 1',
              )
              .first('latest_id'),
            sent.id,
          );
          ...
          assert.equal(
            receivedLetter(sent.recipient, destination)?.id,
            sent.id,
          );
