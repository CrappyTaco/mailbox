...
1. **Sliced envelope:** mailboxPassagePath() combined an approach rectangle ending at local x=3.09 with an aperture beginning at x=7.88. The 4.79-unit gap erased an internal strip during insertion/extraction. MAILBOX_PASSAGE_FACE now defines one connected transit plane extending the opening leftward; the runtime envelope no longer has a passage clip. A separate source-artwork defect persisted: letter.png was a crop of an already partly concealed envelope. It is now rendered from the complete stationery vector layers at their natural 160:88 aspect ratio. See [the corrected diagnosis](envelope-rendering-diagnosis.md).
